...
function ChannelSheet({ visible, onCancel, onPick, busy, t }) {
  return (
...
// First step of "Forgot password?": confirm the phone (pre-filled from
// whatever was already typed on the login screen, if it looked valid), then
// send an OTP the same way sign-up does. Whether an account exists for this
// number is only checked at the final reset step (see ForgotPasswordForm),
// same as sign-up defers its own "does this already exist" check to
// /auth/register rather than to the OTP request.
export default function ForgotPhoneScreen({ navigation, route }) {
  const { t } = useLanguage();
  ...
  async function handlePick(channel) {
    ...
    try {
      ...
      navigation.navigate('ForgotOtp', { phone: trimmed, channel });
    } catch (submitError) {
      setSheetVisible(false);
  ...
  return (
    <WizardScaffold
      title={t('forgotPassword.phoneTitle')}
      subtitle={t('forgotPassword.phoneSubtitle')}
