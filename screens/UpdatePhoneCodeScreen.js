...
export default function UpdatePhoneCodeScreen({ navigation, route }) {
  const { newPhone, channel } = route.params;
  ...
  async function handleVerify() {
    ...
    setError(null);
    try {
      const { verificationToken } = await verifyOtp(newPhone, code);
      const { user: updated } = await changePhoneNumber(newPhone, verificationToken, token);
      await updateUser(updated);
      navigation.replace('UpdateSuccess', { kind: 'phone' });
    } catch (verifyError) {
  ...
  async function handleResend() {
    setError(null);
    try {
      await requestOtp(newPhone, channel);
      setCode('');
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <Text style={styles.title}>{t('updatePhoneCode.title')}</Text>
        <Text style={styles.subtitle}>
          {t(channel === 'sms' ? 'updatePhoneCode.subtitlePhone' : 'updatePhoneCode.subtitleWhatsapp', {
